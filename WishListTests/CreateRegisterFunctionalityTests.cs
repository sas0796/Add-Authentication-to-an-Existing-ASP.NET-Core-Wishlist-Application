...
namespace WishListTests
{
    ...
    {
        [Fact(DisplayName = "Create ApplicationUser @create-ApplicationUser")]
        public void CreateRegisterModel()
        {
            var filePath = ".." + Path.DirectorySeparatorChar + ".." + Path.DirectorySeparatorChar + ".." + Path.DirectorySeparatorChar + ".." + Path.DirectorySeparatorChar + "WishList" + Path.DirectorySeparatorChar + "Models" + Path.DirectorySeparatorChar + "AccountViewModels" + Path.DirectorySeparatorChar + "ApplicationUser.cs";
            Assert.True(File.Exists(filePath), @"`ApplicationUser.cs` was not found in the `Models\AccountViewModels` folder.");

            var ApplicationUser = TestHelpers.GetUserType("WishList.Models.AccountViewModels.ApplicationUser");
            Assert.True(ApplicationUser != null, "A `public` class `ApplicationUser` was not found in the `WishList.Models.AccountViewModels` namespace.");

            var emailProperty = ApplicationUser.GetProperty("Email");
            Assert.True(emailProperty != null, "`ApplicationUser` does not appear to contain a `public` `string` property `Email`.");
            Assert.True(emailProperty.PropertyType == typeof(string), "`ApplicationUser` has a property `Email` but it is not of type `string`.");
            Assert.True(Attribute.IsDefined(emailProperty, typeof(RequiredAttribute)), "`ApplicationUser` has a property `Email` but it doesn't appear to have a `Required` attribute.");
            Assert.True(emailProperty.CustomAttributes.FirstOrDefault(e => e.AttributeType == typeof(EmailAddressAttribute)) != null, "`ApplicationUser` has a property `Email` but it doesn't appear to have an `EmailAddress` attribute.");

            var passwordProperty = ApplicationUser.GetProperty("Password");
            Assert.True(passwordProperty != null, "`ApplicationUser` does not appear to contain a `public` `string` property `Password`.");
            Assert.True(passwordProperty.PropertyType == typeof(string), "`ApplicationUser` has a property `Password` but it is not of type `string`.");
            Assert.True(passwordProperty.CustomAttributes.FirstOrDefault(e => e.AttributeType == typeof(RequiredAttribute)) != null, "`ApplicationUser` has a property `Password` but it doesn't appear to have a `Required` attribute.");
            var stringLength = passwordProperty.CustomAttributes.FirstOrDefault(e => e.AttributeType == typeof(StringLengthAttribute));
            Assert.True(stringLength != null, "`ApplicationUser` has a property `Password` but it doesn't appear to have a `StringLength` attribute of 100, with a minimum length of 8");
            Assert.True(stringLength.ConstructorArguments.Count == 1 && (int)stringLength.ConstructorArguments[0].Value == 100, "`ApplicationUser` has a property `Password`and a `StringLength` attribute, but it's `maxLength` isn't set to 100.");
            Assert.True(stringLength.NamedArguments.Count == 1 && (int)stringLength.NamedArguments[0].TypedValue.Value == 8, "`ApplicationUser` has a property `Password` but it doesn't appear to have a `StringLength` attribute of 100, but it's minimum length was not set to 8");
            var dataType = passwordProperty.CustomAttributes.FirstOrDefault(e => e.AttributeType == typeof(DataTypeAttribute));
            Assert.True(dataType != null, "`ApplicationUser` has a property `Password` but it doesn't appear to have a `DataType` attribute set to `Password`.");
            Assert.True(dataType.ConstructorArguments.Count == 1 && (int)dataType.ConstructorArguments[0].Value == 11, "`ApplicationUser` has a property `Password` and it has a `DataType` attribute but it's not set to `DataType.Password`.");

            var confirmPasswordProperty = ApplicationUser.GetProperty("ConfirmPassword");
            Assert.True(confirmPasswordProperty != null, "`ApplicationUser` does not appear to contain a `public` `string` property `ConfirmPassword`.");
            Assert.True(confirmPasswordProperty.PropertyType == typeof(string), "`ApplicationUser` has a property `ConfirmPassword` but it is not of type `string`.");
            dataType = confirmPasswordProperty.CustomAttributes.FirstOrDefault(e => e.AttributeType == typeof(DataTypeAttribute));
            Assert.True(dataType != null, "`ApplicationUser` has a property `ConfirmPassword` but it doesn't appear to have a `DataType` attribute set to `DataType.Password`.");
            Assert.True(dataType.ConstructorArguments.Count == 1 && (int)dataType.ConstructorArguments[0].Value == 11, "`ApplicationUser` has a property `ConfirmPassword` and it has a `DataType` attribute but it's not set to `DataType.Password`.");
            var confirm = confirmPasswordProperty.CustomAttributes.FirstOrDefault(e => e.AttributeType == typeof(CompareAttribute));
            Assert.True(confirm != null, "`ApplicationUser` has a property `ConfirmPassword` but it doesn't appear to have a `Compare` attribute set to `Password`.");
            Assert.True(confirm.ConstructorArguments.Count == 1 && (string)confirm.ConstructorArguments[0].Value == "Password", "`ApplicationUser` has a property `ConfirmPassword` with a `Compare` attribute, but it is not set to `Password`.");
        }
        ...
        public void CreateRegisterViewTest()
        {
            ...
            var pattern = @"@model\s*WishList[.]Models[.]AccountViewModels[.]ApplicationUser";
            var rgx = new Regex(pattern);
            Assert.True(rgx.IsMatch(file), @"`Register.cshtml` doesn't have it's model set to `WishList.Models.AccountViewModels.ApplicationUser`.");
        ...
        public void CreateHttpPostRegisterActionTest()
        {
            ...
            var method = accountController.GetMethod("Register", new Type[] { ApplicationUser });
            Assert.True(method != null, "`AccountController` did not contain a `Register` method with a parameter of type `ApplicationUser`.");
            ...
            var model = Activator.CreateInstance(ApplicationUser, null);
            ApplicationUser.GetProperty("Email").SetValue(model, "Test@Test.com");
            ApplicationUser.GetProperty("Password").SetValue(model, "success");
            ApplicationUser.GetProperty("ConfirmPassword").SetValue(model, "success");
        ...
        public void AddModelStateCheckToHttpPostRegisterActionTest()
        {
            ...
            var model = Activator.CreateInstance(ApplicationUser, null);
            ApplicationUser.GetProperty("Email").SetValue(model, "Test@Test.com");
            ApplicationUser.GetProperty("Password").SetValue(model, "failure");
            ApplicationUser.GetProperty("ConfirmPassword").SetValue(model, "failure");
